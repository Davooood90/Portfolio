...
function toImagePath(hobby: string) {
  return `/hobbies/${hobby.toLowerCase()}.png`;
}
...
function HobbyPill({ hobby, hasHover }: { hobby: string; hasHover: boolean }) {
  ...
  return (
    ...
    >
      ...
      {cursor && (
        <div
          className="pointer-events-none absolute z-10 h-30 w-30 rounded-md border border-border-strong bg-surface bg-cover bg-center shadow-lg"
          style={{
            left: cursor.x + 12,
            top: cursor.y + 12,
...
export default function HobbiesSection() {
  ...
  return (
    <section
      id="hobbies"
      className="flex flex-col justify-center pt-8 mb-12 font-mono sm:px-8 sm:pt-10 sm:mb-32"
    >
